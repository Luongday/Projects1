...
namespace QuanLyHoSoSinhVien.PresentationLayer.Controller.HoSoController
{
    public class ProfileComandControllerImpl : IDeleteProfileController, IEditProfileController
    {
        ...
        public string DeleteProfile(string mahs)
        {
            ...
            if (string.IsNullOrWhiteSpace(mahs) || string.IsNullOrEmpty(mahs)) return "Mã hồ sơ không hợp lệ!";
        ...
        {
            if (hs == null || string.IsNullOrWhiteSpace(hs.mahs) || string.IsNullOrEmpty(hs.mahs)) return "Dữ liệu không hợp lệ!";
            if (string.IsNullOrWhiteSpace(hs.masv) || string.IsNullOrEmpty(hs.masv))
